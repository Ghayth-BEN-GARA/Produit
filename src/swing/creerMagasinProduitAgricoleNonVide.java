...
public class creerMagasinProduitAgricoleNonVide {

	private JFrame creerMagasinProduitAgricoleNonVideFrame;
	...
	private JComboBox<ProduitAgricole> produitAgricoleComboBox;
	private DefaultListCellRenderer centerDefaultListCellRenderer;
	
	private static List<MagasinProduitAgricole> listeDesMagasins = new ArrayList<MagasinProduitAgricole>();
	private List<ProduitAgricole> listeDesProduitsAffecter;
	
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					creerMagasinProduitAgricoleNonVide window = new creerMagasinProduitAgricoleNonVide();
					window.creerMagasinProduitAgricoleNonVideFrame.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public creerMagasinProduitAgricoleNonVide() {
		initialize();
	}

	private void initialize() {
		creerMagasinProduitAgricoleNonVideFrame = new JFrame("Créer un magasin non vide");
		creerMagasinProduitAgricoleNonVideFrame.setBackground(Color.WHITE);
		creerMagasinProduitAgricoleNonVideFrame.setIconImage(Toolkit.getDefaultToolkit().getImage(menuApplication.class.getResource("/images/gestion.png")));
		creerMagasinProduitAgricoleNonVideFrame.getContentPane().setForeground(Color.BLACK);
		creerMagasinProduitAgricoleNonVideFrame.getContentPane().setBackground(Color.WHITE);
		creerMagasinProduitAgricoleNonVideFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		creerMagasinProduitAgricoleNonVideFrame.setType(Type.POPUP);
		creerMagasinProduitAgricoleNonVideFrame.setAutoRequestFocus(true);
		creerMagasinProduitAgricoleNonVideFrame.setUndecorated(false);
		creerMagasinProduitAgricoleNonVideFrame.setEnabled(true);
		creerMagasinProduitAgricoleNonVideFrame.getContentPane().setLayout(null);
		creerMagasinProduitAgricoleNonVideFrame.setVisible(true);
		creerMagasinProduitAgricoleNonVideFrame.setSize(1000, 800);
		creerMagasinProduitAgricoleNonVideFrame.setResizable(true);
		creerMagasinProduitAgricoleNonVideFrame.setLocationRelativeTo(null);
		creerMagasinProduitAgricoleNonVideFrame.setPreferredSize(new Dimension(1000, 1300));
		
		generalFramePanel = new JPanel();
		generalFramePanel.setBackground(Color.WHITE);
		generalFramePanel.setBounds(0, 0, 984, 749);
		creerMagasinProduitAgricoleNonVideFrame.getContentPane().add(generalFramePanel);
		generalFramePanel.setLayout(null);
		creerMagasinProduitAgricoleNonVideFrame.requestFocusInWindow();
		...
		bgImagelabel.setIcon(new ImageIcon(creerMagasinProduitAgricoleNonVide.class.getResource("/images/fruitBackground.jpg")));
		...
		produitAgricoleComboBox = new JComboBox<ProduitAgricole>();
		...
		produitAgricoleComboBox.setRenderer(centerDefaultListCellRenderer);
		produitAgricoleComboBox.setEditable(false);
		produitAgricoleComboBox.setEnabled(true);
		produitAgricoleComboBox.setFont(new Font("Dialog", Font.PLAIN, 11));
		produitAgricoleComboBox.setBackground(Color.WHITE);
		produitAgricoleComboBox.setBounds(131, 392, 283, 41);
		produitAgricoleComboBox.setForeground(new Color(102, 102, 102));
		formCreerMagasinNonVidePanel.add(produitAgricoleComboBox);
		produitAgricoleComboBox.setMaximumRowCount(20);
		produitAgricoleComboBox.setBorder(new LineBorder(Color.GRAY, 1));
		produitAgricoleComboBox.setUI(new BasicComboBoxUI() {
	...
	public void deconnexion() {
		int dialog = JOptionPane.showConfirmDialog(creerMagasinProduitAgricoleNonVideFrame,"Vous êtes sur de quitter votre session ?", "Déconnexion",
				JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
	    if(dialog == JOptionPane.YES_OPTION){
	    	creerMagasinProduitAgricoleNonVideFrame.dispose();
	...
	public void remplirComboParListeDesProduits() {
		for(ProduitAgricole listePr : creerProduitAgricoleNonVide.getListeDesProduitsAgricoles()) {
			produitAgricoleComboBox.addItem(listePr);
		}
	}
	...
	public void validateFormCreerMagasin() {
		...
		if(validerIdentifiant(identifiant) == false) {
			...
			creerMagasinAgricoleNonVide(Integer.parseInt(identifiant), adresse, Integer.parseInt(capacite));
			nbrMagasinNonVideCreerLabel.setText("+ " + MagasinProduitAgricole.getNombreMagasin() + " " + "magasin(s) non vide(s) créé(s).");
			Timer t = new Timer(3000, new ActionListener() {
	            @Override
	            public void actionPerformed(ActionEvent e) {
	...
	public void creerMagasinAgricoleNonVide(int identifiant, String adresse, int capacite) {
		MagasinProduitAgricole magasinNonVide = new MagasinProduitAgricole(identifiant, adresse, capacite, null);
		listeDesMagasins.add(magasinNonVide);
	}
	
	public void ajouterProduitDansListeDesProduitsAffecter() {
		listeDesProduitsAffecter.add((ProduitAgricole) produitAgricoleComboBox.getSelectedItem());
	}
	
	public void affecterListeDesProduitsAMagasin() {
		for(MagasinProduitAgricole listeM: listeDesMagasins) {
			if(listeM.getIdentifiantMagasin() == Integer.parseInt(identifiantCreerMagasintTextField.getText())){
				listeM.setEnsembleDesProduitsAgricole(listeDesProduitsAffecter);
			}
		}
	}
	
	public void retourAuMenu() {
		int dialog = JOptionPane.showConfirmDialog(creerMagasinProduitAgricoleNonVideFrame,"Retour au menu pricipale ?", "Retour",
				JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
	    if(dialog == JOptionPane.YES_OPTION){
	    	creerMagasinProduitAgricoleNonVideFrame.dispose();
	...
	public static List<MagasinProduitAgricole> getListeDesMagasins() {
